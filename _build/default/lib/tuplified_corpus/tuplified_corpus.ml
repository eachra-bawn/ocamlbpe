  ...
  let rec tuplify_impl (corpus : Corpus.t) acc n =
    if n = Corpus.length corpus then List.rev acc
      ...
      let rec tuplify_cv_val acc2 n2 =
        if n2 = Corpus.Value.length current_cv_val - 1 then List.rev acc2
        else
          let tuplify_current_cv_val =
            (Corpus.Value.nth current_cv_val n2, Corpus.Value.nth current_cv_val (n2 + 1))
          in
...
let pretty_print (tuplified_corpus : t) =
  ...
  in
  Format.printf "tuplified_corpus:\n@[<h>@;<0 2>%a@;<0 0>@]\n\n"
    pp_corpus_tuplifed tuplified_corpus
